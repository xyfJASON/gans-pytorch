...
def train(args, cfg):
    ...
    def run_step_D(X, y):
        ...
        fake = G(z, y).detach()
        loss_disc, loss_cls = loss_fn.forward_D(fake, X, y)
        loss = loss_disc + loss_cls
        accelerator.backward(loss)
        optimizer_D.step()
        return dict(
            loss_disc_D=loss_disc.item(),
            loss_cls_D=loss_cls.item(),
    ...
    def run_step_G(batch_size):
        ...
        fake = G(z, y)
        loss_disc, loss_cls = loss_fn.forward_G(fake, y)

        # if (step + 1) % 300 == 0:
        #     os.makedirs('./tmp/', exist_ok=True)
        #     for idx, (f, label) in enumerate(zip(fake, y)):
        #         f = (f + 1) / 2
        #         import torchvision.transforms as T
        #         T.ToPILImage()(f.cpu()).save(f'./tmp/{step}_{idx}_{label}.png')
        #         if idx == 9:
        #             break
        ...
        optimizer_G.step()
        return dict(
            loss_disc_G=loss_disc.item(),
            loss_cls_G=loss_cls.item(),
