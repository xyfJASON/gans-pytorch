...
def train(args, cfg):
    ...
    ema_G = EMA(G.parameters(), decay=cfg.train.ema_decay)
    ...
    def run_step_D(X):
        ...
        z = torch.randn((X.shape[0], len(cfg.G.params.dim_mults), cfg.G.params.n_basis), device=device)
        x = torch.randn((X.shape[0], cfg.G.params.noise_dim), device=device)
        fake = G(x, z).detach()
    ...
    def run_step_G(batch_size):
        ...
        z = torch.randn((batch_size, len(cfg.G.params.dim_mults), cfg.G.params.n_basis), device=device)
        x = torch.randn((batch_size, cfg.G.params.noise_dim), device=device)
        fake, ortho_reg = G(x, z, return_ortho_reg=True)
        loss = loss_fn.forward_G(fake)
        loss = loss + cfg.train.ortho_reg * ortho_reg
        accelerator.backward(loss)
        optimizer_G.step()
        ema_G.update(G.parameters())
        return dict(loss_G=loss.item(), ortho_reg=ortho_reg.item(), lr_G=optimizer_G.param_groups[0]['lr'])
    ...
    def sample(savepath: str):
        ...
        z = torch.randn((cfg.train.n_samples, len(cfg.G.params.dim_mults), cfg.G.params.n_basis), device=device)
        x = torch.randn((cfg.train.n_samples, cfg.G.params.noise_dim), device=device)
        samples = unwrapped_G(x, z).cpu()
    ...
    def evaluate():
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for n in tqdm.tqdm(
                ...
                z = torch.randn((n, len(cfg.G.params.dim_mults), cfg.G.params.n_basis), device=device)
                x = torch.randn((n, cfg.G.params.noise_dim), device=device)
                samples = unwrapped_G(x, z).cpu()
                for x in samples:
