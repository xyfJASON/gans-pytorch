...
def train(args, cfg):
    ...
    R = instantiate_from_config(cfg.R)
    ...
    cfg.train.optim_R.params.update({'params': R.parameters()})
    optimizer_R = instantiate_from_config(cfg.train.optim_R)
    ...
    def load_ckpt(ckpt_path: str):
        ...
        R.load_state_dict(ckpt_model['R'])
        ...
        optimizer_R.load_state_dict(ckpt_optimizer['optimizer_R'])
    ...
    def save_ckpt(save_path: str):
        ...
        accelerator.save(dict(
            ...
            R=accelerator.unwrap_model(R).state_dict(),
    ...
    G, D, R, optimizer_G, optimizer_D, optimizer_R, train_loader = \
        accelerator.prepare(G, D, R, optimizer_G, optimizer_D, optimizer_R, train_loader)  # type: ignore
    ...
    def run_step_D(X):
        ...
        rec = R(X).detach()
        d_fakez_realx = D(X, rec + torch.randn_like(rec))

        z = torch.randn((X.shape[0], cfg.G.params.z_dim), device=device)
        fake = G(z).detach()
        d_realz_fakex = D(fake, z)

        loss = (bce_with_logits(d_realz_fakex, torch.ones_like(d_realz_fakex)) +
    ...
    def run_step_GR(batch_size):
        ...
        loss = loss_G + cfg.train.lambda_rec * loss_rec
        accelerator.backward(loss)
        optimizer_G.step()
        optimizer_R.step()
        return dict(
    ...
    while step < cfg.train.n_steps:
        ...
        train_status = run_step_GR(batch_size_per_process)
        status_tracker.track_status('Train', train_status, step)
        accelerator.wait_for_everyone()
